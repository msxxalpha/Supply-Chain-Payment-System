...
public class ReportsController(AppDbContext db):Controller
{
 public async Task<IActionResult> Index()
 {
  ...
  var maxRemainingSupplier=suppliers.Select(x=>x.Remaining).DefaultIfEmpty(1).Max();var maxRemainingPart=parts.Select(x=>x.Remaining).DefaultIfEmpty(1).Max();
  suppliers=suppliers.Select(x=>x with{Priority=PriorityScore(x.Remaining,maxRemainingSupplier,x.Coverage,x.Overdue,x.Count)}).ToList();
  parts=parts.Select(x=>x with{Priority=PriorityScore(x.Remaining,maxRemainingPart,x.Coverage,x.Overdue,x.Count)}).ToList();
  var supplierParts=approved.GroupBy(x=>new{x.SupplierId,x.SupplierTitle,x.PartId,x.PartTitle}).Select(g=>{var o=g.Sum(x=>x.OriginalDebt);var a=g.Sum(x=>x.AllocatedAmount);return new SupplierPartRow(g.Key.SupplierTitle,g.Key.PartTitle,g.Count(),o,a,Math.Max(0,o-a),o>0?a/o:0);}).OrderByDescending(x=>x.Remaining).ToList();
  ...
  return View(new ReportsVm(runs.Count,runs.Count(x=>x.Status==PaymentRunStatus.Approved),runs.Count(x=>x.Status==PaymentRunStatus.Cancelled),approved.Count,original,allocated,remaining,original>0?allocated/original:0,suppliers,parts,supplierParts,status,trend,recent));
 }
 static decimal PriorityScore(decimal remaining,decimal maxRemaining,decimal coverage,int overdue,int count){if(remaining<=0)return 0;var debt=maxRemaining>0?remaining/maxRemaining:0;var age=debt;var overdueRate=count>0?(decimal)overdue/count:0;var under=1-Math.Clamp(coverage,0,1);var volume=Math.Min(1,count/20m);return Math.Round(100*(debt*.30m+age*.30m+overdueRate*.20m+under*.15m+volume*.05m),1);}
